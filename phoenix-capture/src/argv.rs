...
pub fn recover_argv(pane_pids: &[u32]) -> HashMap<u32, NonEmpty<String>> {
    ...
    pane_pids
        .iter()
        .filter_map(|&pid| {
            let row = foreground_row(pid, &table)?;
            // `parse_ps_line` already refused an empty command column, so
            // this is the type stating what that check established.
            let argv = NonEmpty::from_vec(split_command(&row.command))?;
            Some((pid, argv))
        })
        .collect()
}
