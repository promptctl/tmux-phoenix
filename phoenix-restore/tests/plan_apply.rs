...
/// tmux resolves a new pane's `pane_current_path` shortly *after*
/// `split-window` returns, so reading it immediately races the server
/// (tmux-flake-4vk). Poll, bounded, until every pane of `target` reports a
/// path; a timeout fails loudly rather than asserting on unsettled state.
fn wait_for_pane_paths<T: Transport>(client: &mut Client<T>, target: &str) {
    for _ in 0..50 {
        let paths = query(
            client,
            "list-panes",
            ["-t", target, "-F", "#{pane_current_path}"],
        );
    ...
    panic!("tmux never resolved every pane's cwd in {target} within 5s");
}
...
fn a_planned_multi_window_multi_pane_tree_applies_cleanly_to_a_live_server() {
    ...
    wait_for_pane_paths(&mut client, "restored:5");
    let panes = query(
        &mut client,
        "list-panes",
        [
