...
use phoenix_core::{
    ...
    SessionName, Snapshot, TmuxVersion, Utf8PathBuf, Window, WindowIndex, WindowName,
};
...
fn write_option<T>(
    w: &mut Writer,
    value: Option<&T>,
    write: impl FnOnce(&mut Writer, &T) -> Result<(), StoreError>,
) -> Result<(), StoreError> {
    match value {
        None => {
            w.write_u8(0);
...
fn encode_pane(w: &mut Writer, pane: &Pane, blobs: &BlobStore) -> Result<(), StoreError> {
    ...
    write_option(w, pane.cwd.as_ref(), |w, cwd| {
        w.write_str(cwd.as_str());
        Ok(())
    })?;
    w.write_str(pane.program.command.as_str());
    write_option(w, pane.program.argv.as_ref(), |w, argv| {
        let args: Vec<&String> = argv.iter().collect();
        w.write_vec(&args, |w, arg| w.write_str(arg));
        Ok(())
    })?;
    write_option(w, pane.content.as_ref(), |w, content| {
...
fn decode_pane(r: &mut Reader, blobs: &BlobStore) -> Result<Pane, StoreError> {
    ...
    let cwd = read_option(r, |r| {
        Utf8PathBuf::parse(r.read_str()?).ok_or(StoreError::InvalidName)
    })?;
    let command = ProgramName::parse(r.read_str()?).ok_or(StoreError::InvalidName)?;
    let argv = read_option(r, |r| {
        NonEmpty::from_vec(r.read_vec(Reader::read_str)?).ok_or(StoreError::EmptyCollection)
    })?;
    let content = read_option(r, |r| {
    ...
    Ok(Pane {
        ...
        index,
        cwd,
        program: CapturedProgram { command, argv },
        content,
    })
}
...
mod tests {
    ...
    fn sample_snapshot() -> Snapshot {
        ...
        let pane1 = Pane {
            id: PaneId(9),
            index: PaneIndex(1),
            cwd: Utf8PathBuf::parse("/home/user/proj"),
            program: CapturedProgram {
