...
layout, working directories, active window — as new tmux sessions, onto a running server or
one with no sessions. `--dry-run` prints the tmux commands a real restore would run without
running any of them (scrollback replay shows as a summary line, since its temp file only
exists at apply time), a safety property tmux-resurrect doesn't have (it has no preview mode
at all). Captured scrollback is replayed
...
| Restore preview before touching a live session | None | `--dry-run` (scrollback replay previews as a summary line) |
